...
int main(int argc, char *argv[])
{
    ...
    while (runTime.run())
    {
        ...
        if (adjustTimeStep && mesh.changing())
        {
            ...
            scalar maxDeltaTMesh = 0.9/(meshCoNum + SMALL);
            scalar deltaTMesh = min(min(maxDeltaTMesh, 1.0 + 0.1*maxDeltaTMesh), 1.2);

            runTime.setDeltaT
            (
                min
                (
                    min(deltaTFact*runTime.deltaTValue(),
                        deltaTMesh*runTime.deltaTValue()),
                    maxDeltaT
                )
            );
