...
#include "fishBodyMotionSolver.H"
...
namespace Foam
{
    defineTypeNameAndDebug(fishBodyMotionSolver, 0);
...
Foam::fishBodyMotionSolver::fishBodyMotionSolver
(
    const polyMesh& mesh,
...
:
    ...
    SBMFPtr_(fishBodyMotionFunction::New(coeffDict(), mesh.time())),
    pointIDs_(),
    moveAllCells_(false)
{
    word cellZoneName =
    ...
    {
        Info<< "Applying fish body motion to cellZone " << cellZoneName
            << endl;
    ...
    if (cellSetName != "none")
    {
        Info<< "Applying fish body motion to cellSet " << cellSetName
            << endl;
    ...
    if (moveAllCells_)
    {
        Info<< "Applying fish body motion to entire mesh" << endl;
    }
    else
    {
