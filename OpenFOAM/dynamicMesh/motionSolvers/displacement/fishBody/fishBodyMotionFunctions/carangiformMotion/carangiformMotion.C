...
transformationPoints(pointField& p0) const
{
    ...
    forAll(p0, pointI)
    {
        ...
        const scalar localAmplitude = amplitude_[0] + (amplitude_[1]*x) + (amplitude_[2]*x*x);
        const scalar dampFactor = 0.5-0.5*tanh(ramp_*x-(ramp_+9));
                    
        const scalar yr = y + localAmplitude * dampFactor * sin(omega_*tm - waveNumber_*x) * length_;
